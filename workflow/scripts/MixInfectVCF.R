...
MixInfectVCF = function(vcfInput,outputfile,LowCov=10){
  ...
  if (length(mixnames)>0){
    ...
    mixoutfile<-outfile
    #write.table(mixoutfile,paste0(outputfile,"_MixedSampleSummary.txt"),row.names = F,sep="\t")
    write.table(mixoutfile,outputfile,row.names = F,sep = "\t",quote = F)
...
MixInfectVCF(vcfInput, outputfile)
